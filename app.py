...
# 1. KONFIGURACE API KLÍČŮ (Zde doplň své údaje nebo použij Streamlit Secrets)
TMDB_API_KEY = "TVUJ_TMDB_API_KLIC"
NOTION_TOKEN = "ntn_p75258623695St2zIkmATryx6pmfPexDXD3gEjupbLs01a"
NOTION_DATABASE_ID = "ID_TVOJE_NOTION_DATABAZE"
# Inicializace TMDB
tmdb = TMDb()
...
"type": "paragraph",
"paragraph": {
"rich_text": [{ "type": "text", "text": { "content": zkraceny_popis if
zkraceny_popis else "Bez popisu." } }]
}
}
]
}
...
response = requests.post("https://api.notion.com/v1/pages", json=payload,
headers=headers)
return response
...
2. Seznam knihoven (`requirements.txt`)
Vytvoř druhý soubor s názvem requirements.txt a vlož do něj:
if query:
...
poster_url = f"https://image.tmdb.org/t500{details.poster_path}" if
details.poster_path else None
with col1:
if poster_url:
...
st.write(details.overview if details.overview else "Český popis není k
dispozici.")
